...
def get_odds_data():
    ...
    Llama a la función que ahora solo devuelve un valor.
    """
...
if st.button('Obtener Cuotas Ahora'):
    with st.spinner('Buscando en la web... Este proceso puede tardar hasta un minuto.'):
        # La llamada ahora solo espera y recibe un único valor: los datos.
        datos = get_odds_data()
        ...
        if datos:
            ...
            st.error('No se pudieron obtener los datos. El sitio web podría estar bloqueando el acceso o no hay carreras disponibles.')
